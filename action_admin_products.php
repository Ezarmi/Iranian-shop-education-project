...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'DELETE':
            ...
            if (mysqli_query($link, $query) === true) {
                $file = "assets/images/products/$pro_image";
                if (unlink($file))
...
$check = getimagesize($_FILES['pro_image']["tmp_name"]); // نوع پرونده تصویری را برمی‌گرداند
if ($check !== false) {
    echo "پرونده انتخابی یک تصویر از نوع - " . $check["mime"] . " است <br />";
    $uploadOk = 1;
} else {
    echo "پرونده انتخاب شده یک تصویر نیست <br />";
    // در صورتی که محتوای پرونده ارسال شده تصویر نباشد خروجی تابع getimagesize مقدار منطقی false خواهد بود.
    $uploadOk = 0;
}
...
//     if (move_uploaded_file($_FILES["pro_image"]["tmp_name"], $file)) {
